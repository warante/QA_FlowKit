...
Activated in governed sync mode (it is not part of the numbered main sequence in `qa-workflow-orchestrator.md`). It runs two governed sub-steps:

- Sync Apply, after the sync plan and diff are approved and the rollback plan exists.
- Sync Verify, to capture a post-apply snapshot.

Skipped if `testManagementSync.mode` is `"proposal-only"`.

## Inputs

- `.qa-ai/output/test-management-sync-diff.md` (approved sync diff from the governed diff sub-step).
