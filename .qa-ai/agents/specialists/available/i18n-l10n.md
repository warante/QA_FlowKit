...
- **Strategy family:** `i18n-l10n`.
- **Allowed evidence types:** `test-plan`, `manual-charter`, `automation-script`, `residual-risk`.
- **Optional auxiliary artifact:** `.qa-ai/output/localization-test-matrix.md`.
- **Create it only when:** multiple locales, languages or regional formatting are in scope.
