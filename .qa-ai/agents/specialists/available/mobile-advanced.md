...
- Do **not** load solely because the project uses Appium, Maestro or BrowserStack — use `appium.md`, `maestro.md` or `browserstack-strategy.md` for framework and cloud execution setup.
- Load with `browserstack-strategy` when device-cloud execution is required for camera, geolocation, biometrics or real-device evidence.
...
- Route cloud execution details to `browserstack-strategy` when BrowserStack or App Automate is the platform.
