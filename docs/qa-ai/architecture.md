...
AGENTS.md                       generic instruction layer for AI agents
docs/qa/                        generated workflow artifacts
...
- `init.mjs` reads a preset, generates `qa-ai.config.yaml`, creates configured docs/features/tests folders, writes starter QA artifacts, and syncs agent adapters without overwriting existing files unless `--force` is passed.
- `doctor.mjs` validates the framework folder, required scripts/rules/templates/agents/presets/adapters, generated target files and configured paths. Missing framework config files are warnings.
...
- `validate-features.mjs` scans configured `.feature` files for one scenario, acceptance criteria, required tags and RF IDs.
...
- Requirements source: Jira story when available.
- Complementary sources: PRD, RF markdown, Confluence and attachments.
- Test design source: versioned `.feature` files in the repository.
- Test management: TestRail for execution/reporting, not as the only source of truth.
- Traceability: `docs/qa/traceability-matrix.md`.
...
qa-workflow-orchestrator
  ...
  -> webdriverio-implementation-agent
  -> api-testing-agent
  -> jira-task-agent
  -> pr-agent
```
