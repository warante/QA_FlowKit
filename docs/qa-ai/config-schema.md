...
truth is [`.qa-ai/contracts/config.v1.schema.json`](../../.qa-ai/contracts/config.v1.schema.json). Values shown are
typical defaults; presets override paths and tool names. Presets may keep `CHANGE_ME` as an internal sentinel, but
generated `qa-ai.config.yaml` files must not contain it. `init` derives supported values such as `project.name` and
`testrail.projectName`, and fails with the unresolved key paths when any sentinel remains.
